...
class FileServer(BaseHTTPRequestHandler):
    ...
    def serve_static(self, path):
        ...
        try:
            ...
            if if_none_match and (etag == if_none_match.strip() or etag in if_none_match or if_none_match == '*'):
    ...
    def serve_json_dir(self, path, url_path):
        ...
        try:
            ...
            data, is_gzipped = self.compress_if_supported(data, "application/json")
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            if is_gzipped:
                self.send_header("Content-Encoding", "gzip")
...
# Production ready: constant-time auth, realpath security, streaming multipart v1.2.0